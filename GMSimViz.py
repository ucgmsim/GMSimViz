...
def timeslice(job, meta):
    ...
    if os.path.exists(
        "%s/%s.png" % (meta["wd"], os.path.splitext(os.path.basename(gmt_ps))[0])
    ):
    ...
    map_width, map_height = gmt.map_dimentions(
        projection=projection, region=llur, wd=swd
    )
    ...
    if job["tilt"] < meta["map_tilt"]:
        p.spacial("X", (0, 1, -1, 1), sizing="%s/%s" % (PAGE_WIDTH, PAGE_HEIGHT))
        p.path(
            "0 0\n1 0\n1 1\n0 1",
            is_file=False,
            close=True,
    ...
    # srf plane top edges
    p.path(meta["gmt_top"], is_file=False, colour="black", width="2p", z=True)
    ...
    # load srf plane data
        ...
        for i in xrange(meta["n_plane"] * (job["transparency"] < 100)):
            ...
            xyv[:, :2] = gmt.mapproject_multi(
                subfaults[:, :2], wd=swd, z="-Jz%s" % (z_scale), p=True
            )
            xyv[:, 1] += subfaults[:, 2] * z_scale * math.cos(math.radians(job["tilt"]))
            xyv[:, 2] = np.memmap(
            ...
            if rc == gmt.STATUS_INVALID:
                ...
                continue
            # dump as binary
            xyv.astype(np.float32).tofile("%s/slip_%d.bin" % (swd, i))
            # search radius based on diagonal distance
            p2 = xyv[meta["planes"][i]["nstrike"] + 1, :2]
            search = (
                math.sqrt(abs(xyv[0, 0] - p2[0]) ** 2 + abs(xyv[0, 1] - p2[1]) ** 2)
                * 1.1
            )
            del xyv
            rc = gmt.table2grd(
            ...
            if rc == gmt.STATUS_INVALID and os.path.exists("%s/slip_%d.grd" % (swd, i)):
    ...
    if os.path.isfile(gm_file):
        p.overlay3d(
            ...
            transparency=job["transparency"],
            dpi=meta["dpi"],
            z="-Jz%s" % ((meta["gm_z_km"] * -z_scale) / meta["xyts_cpt_max"]),
            mesh=True,
            mesh_pen="0.1p",
        )
    p.rose(
    ...
    llur_i = gmt.mapproject_multi(
        [
            ...
            [PAGE_WIDTH - WINDOW_R, window_b / math.sin(math.radians(job["tilt"]))],
        ],
        inverse=True,
        wd=swd,
    )
    ...
    try:
        assert cpt_label != ""
        p.text(scale_margin, cpt_y, cpt_label, align="RM", dx=-SCALE_PAD, size=16)
    ...
    p.image(
        "R",
        "T",
        os.path.join(os.path.dirname(__file__), "quakecore-logo.png"),
        width="2.5i",
        pos="rel",
        dx=WINDOW_R - 0.15,
        dy=-0.15,
    ...
    ###
    p.spacial(
        "OA%s/%s/%s/" % (lon1, lat1, job["azimuth"] - 90),
        (
            str(llur_i[0][0]),
            str(llur_i[0][1]),
            str(llur_i[1][0]),
...
if len(sys.argv) > 1:
    ...
    def frame_op(op, seq_from, seq_to):
        op(
            "%s/%s_perspective_%.4d.png" % (gmt_temp, basename, seq_from),
    ...
    if args.animate:
        # output movie
        gmt.make_movie(
            "%s/%s_perspective_%%04d.png" % (gmt_temp, basename),
            basename,
            fps=args.framerate,
            codec="libx264",
        )
        ...
        move(
            "%s/%s_perspective.png" % (gmt_temp, basename),
